 ...
 * Defaults to a "diagnostic clarity" visual — left side is a noisy/blurry
 * preliminary read, right side is the clean digital interpretation. Tuned
 * to read as professional cardiac diagnostics for theCLINICS.
 */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #F2EDE3 0%, #DCE4DD 100%)' }}
      >
        <svg viewBox="0 0 200 125" className="w-full h-full">
          <defs>
            <linearGradient id="ba-clean-grid" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="#7A9885" stopOpacity="0.18" />
              <stop offset="100%" stopColor="#7A9885" stopOpacity="0" />
            </linearGradient>
          </defs>
          <rect width="200" height="125" fill="url(#ba-clean-grid)" />
          {/* grid lines */}
          {[20, 40, 60, 80, 100].map((y) => (
            <line key={`h-${y}`} x1="10" x2="190" y1={y} y2={y} stroke="#7A9885" strokeWidth="0.2" opacity="0.45" />
          ))}
          {[20, 50, 80, 110, 140, 170].map((x) => (
            <line key={`v-${x}`} x1={x} x2={x} y1="20" y2="100" stroke="#7A9885" strokeWidth="0.2" opacity="0.45" />
          ))}
          {/* Clean ECG waveform */}
          <path
      ...
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(135deg, #EDE6D6 0%, #E8C5B0 100%)',
        }}
      >
        <svg viewBox="0 0 200 125" className="w-full h-full">
          <defs>
            <pattern id="ba-noise" patternUnits="userSpaceOnUse" width="3" height="3">
              <rect width="3" height="3" fill="#EDE6D6" />
              <circle cx="0.5" cy="0.5" r="0.4" fill="#A85F3F" opacity="0.25" />
              <circle cx="2" cy="2" r="0.3" fill="#8F4A2E" opacity="0.18" />
            </pattern>
          </defs>
          <rect width="200" height="125" fill="url(#ba-noise)" />
          {/* jittery old-paper EKG */}
          <path
      ...
      <div
        ...
        style={{ left: `${pos}%`, background: 'white', boxShadow: '0 0 20px rgba(0,0,0,0.3)' }}
      />
      <div
        ...
        style={{ left: `${pos}%`, boxShadow: '0 8px 24px rgba(31,58,46,0.35)' }}
        onMouseDown={() => (dragRef.current = true)}
      >
      ...
      <div
        ...
        style={{ background: 'rgba(168, 95, 63, 0.95)', color: 'white' }}
      >
        Before
      </div>
      <div
        ...
        style={{ background: 'rgba(122, 152, 133, 0.95)', color: 'white' }}
      >
        After · same visit
      </div>
      <div
